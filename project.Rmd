...
class_gpas <- ds_courses %>% select(`Primary Instructor Name`, `# of Students`, `Course GPA`) %>% 
  group_by(`Primary Instructor Name`) %>% summarize(
    avg_stud = mean(`# of Students`), 
    `avg_gpa` = mean(`Course GPA`)); class_gpas

ggplot(class_gpas, aes(x=avg_gpa, y=avg_stud, color=`Primary Instructor Name`)) + geom_point() +
  labs(title="Average Course GPA vs Class Size by Instructor",
      x="Average Course GPA",
      y="Average Class Size",
      caption="A figure illustrating the correlation between a professor's associated gpa
      and the size of the classes they teach.") +
