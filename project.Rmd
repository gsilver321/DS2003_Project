...
```{r}
# Q1
ggplot(ds_courses, aes(x=`Course GPA`, y=`Primary Instructor Name`)) + 
...
ds_courses %>% select(`Primary Instructor Name`, `Course GPA`) %>% group_by(`Primary Instructor Name`) %>%
  summarize(avg_gpa = mean(`Course GPA`)) %>% arrange(desc(avg_gpa))
```
...
ggplot(grpd_courses, aes(x=prob_A, y=`Class Title`)) + geom_col() + xlim(0, 1) +
...
grpd_courses %>% mutate(prob_A) %>% arrange(desc(prob_A))
