...
all_courses <- filter(all_courses, `Catalog Number` < 5000); all_courses 
```
...
all_courses <- mutate(all_courses, Category = case_when(
  Course %in% Fundamental | Course=="DS 1002" ~ "Fundamental",
...
```{r}
# mark ds courses
all_courses <- mutate(all_courses, `Class Type` = if_else(substr(Course,1,2)=="DS", "DS Class", "Outside Class"))
all_courses
```
...
```{r}
ggplot(all_courses, aes(fill=`Class Type`, y=`Course GPA`, x=Subject)) + 
  geom_boxplot() + ggtitle("Course GPA Difference Between DS and non-DS Courses \nGrouped By Course Subjects")
```
